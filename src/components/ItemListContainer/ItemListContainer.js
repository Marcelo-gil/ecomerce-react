...
const traerProductosApi = async () => {
    ...
    informacion.forEach((item) => {
        ...
        productosApi[xid] = { id: item.id,nombre: item.title.toUpperCase() , sku: item.id , precio: item.price , stock: xstock , initial: 1, oferta: false ,"imagenMeli": true , "novedad": false, imagenArt: item.image, rating: item.rating,category: item.category, descripcion: item.description};
        xid += 1;
    });
    return productosApi;
}
